...
public:
    FunctionDefineStatement(const std::string &, const std::list<std::pair<std::string, std::string>> &, 
        Statement *);
    void execute() override;
};

FunctionDefineStatement::FunctionDefineStatement(const std::string &name, 
    const std::list<std::pair<std::string, std::string>> &args, Statement *body)
{
    this->name = name;
    this->args = args;
    this->body = body;
