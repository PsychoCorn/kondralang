...
{
    switch (ListOfIdentifiers::getType(key))
    {
    case Type::None:
...
{
    if (ListOfIdentifiers::getType(key) != Variables<T>::type)
        throw std::runtime_error(ERR_MSG_UNKN_ID);
    variables[key] = value;
}
